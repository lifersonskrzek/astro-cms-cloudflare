...
Base clonável para blogs e sites com CMS completo, pronto para Cloudflare Pages. Inclui admin, API CRUD, auth e 14 plugins pré-instalados. Traga seu design.

## O que é

O **astro-cms-cloudflare** é uma base com toda a infraestrutura CMS já configurada:
...
- Deploy nativo para **Cloudflare Pages** com `nodejs_compat`
...
```bash
# 1. Clone
git clone https://github.com/lifersonskrzek/astro-cms-cloudflare.git meu-blog
cd meu-blog
...
| Redirects | Redirecionamentos 301/302 via `_redirects` do Cloudflare Pages |
...
Em desenvolvimento, os dados são gravados localmente. Em produção, o token GitHub é necessário para persistir dados via GitHub API. O Cloudflare Pages detecta o commit e faz rebuild automático em ~2 minutos.
...
export const TEMPLATE_NAME = 'astro-cms-cloudflare';
...
- Repositório GitHub (para persistência em produção)
