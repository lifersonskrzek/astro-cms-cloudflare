...
export default function RedirectsManager() {
  ...
  const saveRedirects = async (newList: Redirect[]) => {
    ...
    try {
      ...
      triggerToast(`${ativos} redirect${ativos !== 1 ? 's' : ''} ativo${ativos !== 1 ? 's' : ''} — sincronizado com Cloudflare Pages!`, 'success', 100);
    } catch (err: any) {
    ...
    <div className="max-w-4xl space-y-6">
      ...
      {redirects.length > 0 && (
        <div className="bg-emerald-50 rounded-2xl border border-emerald-200 p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-600">Sincronizado automaticamente com o Cloudflare Pages. Funciona assim que o deploy terminar (~2 min).</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-blue-50 rounded-2xl border border-blue-200 p-5">
        ...
        <ul className="space-y-1 text-sm text-blue-800">
          ...
          <li>• Os redirects são aplicados automaticamente no Cloudflare Pages após o deploy</li>
        </ul>
      </div>
    </div>
  );
