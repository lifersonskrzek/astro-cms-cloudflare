...
export async function importWordPressXML(xmlContent: string): Promise<ImportResult> {
    ...
    try {
        ...
        const isServerless = !!(process.env.CF_PAGES || process.env.NETLIFY || process.env.AWS_LAMBDA_FUNCTION_NAME);
        ...
        if (isServerless && !hasGitHubCreds) {
            ...
            result.errors.push(
                ...
                'não estão configuradas. Acesse o painel do Cloudflare Pages → Settings → Environment Variables e ' +
