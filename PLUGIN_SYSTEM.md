...
6. Cloudflare Pages rebuild automático → plugin atualizado em ~2min
...
5. Cloudflare Pages rebuild → plugin funcional
...
1. **Dev vs Prod:** `writeFileToRepo()` usa filesystem local em dev, GitHub API em prod (Cloudflare Pages). Isso significa que em dev as mudanças são imediatas, em prod geram um commit que dispara rebuild.
